...
app.get('/', ensureAuthenticated, (req, res) => {
    ...
    res.redirect(process.env.PUBLIC_URL ? `${process.env.PUBLIC_URL}/login` : '/login');
});
app.use(express.static(path.join(__dirname, 'client/build')));
app.get('/registration', ensureAuthenticated, (req, res) => {
  console.log('Reg REQ', req.user.id);
  let options = {
    maxAge: 1000 * 60 * 15, // would expire after 15 minutes
    httpOnly: false, // The cookie only accessible by the web server    
    path: '/'
  }
  res.cookie('uid', req.user.id, options);
  res.cookie('email', req.user['_json'].email, options);
